...
import type {
  Asset, Market, MarketBook, Order, Outcome, Portfolio, Side, OrderType, PricePoint,
} from "../sim/types";
import { buildMarket, currentWindowOpen, marketId, resolveOutcome, shouldResolve } from "../sim/resolution";
    ...
    (set, get) => ({
      ...
      placeOrder: (input) => {
        ...
        if (order.side === "BUY") {
          ...
          const pos = portfolio.positions.find((p) => p.marketId === order.marketId && p.outcome === order.outcome);
          if (!pos || pos.size < order.size - 1e-9) return { ok: false, message: "Insufficient shares" };
        }
        ...
        let costThisCall = 0;
        if (order.type === "MARKET" || order.type === "FOK" || (order.type === "LIMIT" && !order.postOnly)) {
